...
Конструктор копирования не может быть вызван после создания объекта, так как один объект не может быть создан два раза.
Когда возникает необходимость существующий объект сделать копией другого существующего объекта, то роль конструктора копирования выполняет оператор присваивания.
Оператор присваивания делает то же самое, что и конструктор копирования, но для существующего объекта.

Так же, как и конструктор копирования, оператор присваивания принимает константную ссылку на объект
...
Мы много говорили о том, как можно передавать параметры в функцию. О том, что это можно делать по значению, по указателю, по ссылке, и что при этом происходит.
Но мы ни разу не говорили о том, как значения возвращаются из функции. Значение из функции так же можно вернуть по значению, по указателю, либо по ссылке.

При возврате значения из функции по значению, возвращаемое значение копируется на место вызова и на месте вызова создается временный безымянный объект
и если возвращаемое значение является объектом, то при его возврате вызывается конструктор копирования, для того, чтобы скопировать его нам место вызова.

Временные безымянные объекты существуют в пределах одного выражения и удаляются из памяти, когда выражение отработало.
Временные безымянные объекты, существующие в пределах одного выражения, являются константами.

Для того, чтобы возвращаемое значение не копировалось на место вызова и на месте функции не создавался временный безымянный объект - значение можно возвращать по ссылке.
Тогда на место вызова будет копироваться адрес объекта. Но при возврате по ссылке нужно быть очень осторожным, чтобы не вернуть ссылку на локальный объект, 
который уничтожится по завершении функции, что может привести к непредсказуемым последствиям.
----------------------------------------------------------------------
Перегрузка операторов:

Перегрузка операторов нужна для того, чтобы над объектами можно было выполнять такие же действия, как и над обычными переменными:

    Point A(2, 3);
	Point B(7, 8);
	Point C = A + B;

1) Перегрузить можно только существующие операторы. То есть нельзя создавать новые операторы:
+      перегружается;
++     перегружается;
%      перегружается;
&&     не перегружается;
...
Перегруженные операторы это самые обычные функции, имя которых состоит из ключевого слова "operator" и знака "@"
Операторы можно перегружать как в классе, так и за классом.

Если оператор перегружен в классе, то он является методом и может быть вызван только для какого-то объекта.
Если оператор перегружен за классом, то это самая обычная глобальная функция.
...
Если унарный оператор перегружен за классом, то он всегда принимает только один параметр - свою операнду.
Если бинарный оператор перегружен в классе, то он принимает только один параметр - свой операнд справа, его операндом слева будет объект, для которого он вызывается.
Если бинарный оператор перегружается за классом, то он принимает два параметра - операнд справа и слева.

При перегрузке операторов нужно учитытывать то, что некоторые операнды изменяют свои операнды, а некоторые нет. 
Те операторы, которые изменяют свои операнды - лучше перегружать внутри класса.
*/